...
class CNN() : # to take preprocessed data , pass them into model and print results
    ...
    def fit(self,x_train  , y_train  , x_test, y_test , outputdim) :         
      #Fitting CNN Model
        ...
        model.add(Conv1D(10, kernel_size=3, strides=1,activation='relu',input_shape = ( x_train.shape[1],1)))
        model.add(MaxPooling1D(pool_size=3))
        model.add(Dropout(0.5))
        
        #model.add(BatchNormalization())
        model.add(Conv1D(5, kernel_size=1,activation='relu'))
        ...
        model.summary()
        adm=Adam(learning_rate=0.00001)
...
for met in measures:
    time1=time.time()
    print("###############")
    fname='table_'+dataset+'_CNN_'+met+'_5RE.txt'
    tables = open(fname, 'w')
